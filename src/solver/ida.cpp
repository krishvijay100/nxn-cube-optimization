...
// phase 1: search over all 18 moves
struct Phase1Node {
    Coord co, eo, sl;
};

inline int phase1_h(Phase1Node n) {
    return std::max<int>(corner_ori_slice_pruning()[n.co][n.sl], edge_ori_slice_pruning()[n.eo][n.sl]);
}
...
inline Phase1Node phase1_apply(Phase1Node n, int m) {
    return {
        corner_ori_move_table()[n.co][m],
        edge_ori_move_table()[n.eo][m],
        slice_move_table()[n.sl][m],
    };
}

// phase 2: search over 10 G1 moves
struct Phase2Node {
    Coord cp, ep, sp;
};

inline int phase2_h(Phase2Node n) {
    return std::max<int>(corner_perm_slice_perm_pruning()[n.cp][n.sp], edge_perm_slice_perm_pruning()[n.ep][n.sp]);
}
...
inline Phase2Node phase2_apply(Phase2Node n, int i) {
    return {
        corner_perm_move_table()[n.cp][i],
        edge_perm_ud_move_table()[n.ep][i],
        slice_perm_move_table()[n.sp][i],
    };
}
...
bool phase1_dfs(Phase1Node n, int g, int bound, int& next_bound, int last_face, int last_axis_partner_face, std::vector<int>& path)
{
    int f = g + phase1_h(n);
    if (f > bound) {
    ...
    for (int m = 0; m < NUM_MOVES; ++m) {
        ...
        Phase1Node next = phase1_apply(n, m);
        int new_partner = same_axis(face, last_face) ? last_face : -1;
        path.push_back(m);
        if (phase1_dfs(next, g + 1, bound, next_bound, face, new_partner, path)) {
            return true;
        }
        path.pop_back();
    }
...
bool phase2_dfs(Phase2Node n, int g, int bound, int& next_bound, int last_face, int last_axis_partner_face, std::vector<int>& path)
{
    int f = g + phase2_h(n);
    if (f > bound) {
        if (f < next_bound) next_bound = f;
    ...
    for (int i = 0; i < NUM_G1_MOVES; ++i) {
        ...
        Phase2Node next = phase2_apply(n, i);
        int new_partner = same_axis(face, last_face) ? last_face : -1;
        path.push_back(i);
        if (phase2_dfs(next, g + 1, bound, next_bound, face, new_partner, path)) {
            return true;
        }
        path.pop_back();
    }
...
std::vector<Move> ida_phase1(const CubeState& state) {
    ...
    std::vector<int> path;
    int bound = phase1_h(start);
    while (bound <= MAX_DEPTH_PHASE1) {
        int next_bound = SEARCH_FAILED;
        path.clear();
        if (phase1_dfs(start, 0, bound, next_bound, -1, -1, path)) {
            std::vector<Move> out;
...
std::vector<Move> ida_phase2(const CubeState& state) {
    ...
    std::vector<int> path;
    int bound = phase2_h(start);
    while (bound <= MAX_DEPTH_PHASE2) {
        int next_bound = SEARCH_FAILED;
        path.clear();
        if (phase2_dfs(start, 0, bound, next_bound, -1, -1, path)) {
            std::vector<Move> out;
            out.reserve(path.size());
